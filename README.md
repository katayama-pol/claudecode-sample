...
|---|---|---|---|
| **CLAUDE.md** | プロジェクトの常設メモリ（`@`で他ファイルも取り込み可） | セッション開始時に**自動** | 毎回説明しなくても文脈を共有 |
| **Rules** | 指示を分割したルールファイル（`.claude/rules/`） | セッション開始時 or パスマッチ時に**自動** | ルールを部品化して条件付きロード |
| **Skills** | 定型作業の手順書 | `/名前` または関連時に**自動** | 「いつものやり方」を再現 |
| **Subagents** | 専門タスク用の別エージェント | 委譲時／関連時に**自動** | 別コンテキストで集中処理 |
| **Hooks** | ライフサイクルに割り込むスクリプト | ツール実行・プロンプト送信・停止時など**各種イベントで自動** | 強制・自動化（人手いらず） |
| **Output styles** | システムプロンプトの差し替え | `/output-style` で切替（永続） | エージェントの振る舞い全体を変える |
| **`--append-system-prompt`** | 起動時の一時的な追加指示 | `claude` 起動時の**そのセッション中** | その場限りの方針注入 |
...
```
  ...
  Rules      (常時 or パスマッチ時)  Output style( /output-style )
  Hooks      (各種イベント毎)        --append-system-prompt (起動時)
